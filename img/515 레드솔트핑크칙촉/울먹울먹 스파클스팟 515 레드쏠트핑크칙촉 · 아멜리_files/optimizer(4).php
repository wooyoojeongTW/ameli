...
$(".xans-product-additional .cont h3").click(function() {
  $(this).next(".xans-product-additional .cont span").stop().slideToggle(300);
            ...
            success: function(data) {
                ...
                if (data.is_secret == true) {
                    ...
                    if (data.write_auth == true) {
                        aHtml.push('<a href="/board/product/modify.html?board_act=edit&no='+data.no+'&board_no=4&link_product_no='+iProductNo+'" class="btn_style01 btn_white">게시글 수정하기</a>');
                    }
                    aHtml.push('</span></p>');
                    aHtml.push('</div>');
                    ...
                    if (data.comment_write != undefined) {
                        ...
                        aHtml.push('</div>');
                        aHtml.push(''+data.comment_write['comment']+'<a href="#none" onclick="'+data.comment_write['action_comment_insert']+'" class="btn_style01 btn_white">확인</a></div>');
                    ...
                    if (data.comment_reply != undefined) {
                        ...
                        aHtml.push('<p class="ec-base-help '+data.comment_write['password_rule_help_display_class']+'">영문 대소문자/숫자/특수문자 중 2가지 이상 조합, 10자~16자</p>');
                        aHtml.push('</div>');
                        aHtml.push(''+data.comment_reply['comment']+'<a href="#none" onclick="'+data.comment_reply['action_comment_insert']+'" class="btn_style01 btn_white">확인</a></div>');
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data.is_secret == true) {
                    ...
                    aHtml.push('<p class="ec-base-button"><span class="gRight">');
                    if (data.write_auth == true) {
                        aHtml.push('<a href="/board/product/modify.html?board_act=edit&no='+data.no+'&board_no=6&link_product_no='+iProductNo+'" class="btn_style01 btn_white">게시글 수정하기</a>');
                    }
                    aHtml.push('</span></p>');
                    aHtml.push('</div>');
...
;(function($){
    ...
    function pinterest()
    {
        var pinterest = {c_wid:180, w_wid:0};
...
$(document).ready(function(){
    ...
    cellSize = (isNaN(cellSize) || cellSize <= 0) ? 216 : cellSize;
    ...
    function setEvt()
    {
        ...
        $('.xans-photoreview-display .more a').click(function(e){
            ...
            if (readyFlag == true) {
                readyFlag = false;
                moreBtn.hide();
                $.CAFE24_SDK_REVIEWER_UP.call_more({act:'getMore',pno:gpno, next:gpage+1, cnt:glimit, mode:gtype, point:guse_point, config: config}, page_refresh);
            }
...
//        });
    ...
    function makeCell(c)
    {
        ...
        tmpl.push('<p class="point is_rating"><img src="/web/season2_skin/skin13/images/ico_point'+c.rpoint+'.gif" alt="'+c.rpoint+'점" /></p>');
